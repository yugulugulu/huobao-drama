...
test('settings page exposes official provider templates only', () => {
  ...
  assert.match(settingsPage, /TOKENBOX_BASE_URL = 'https:\/\/tokenbox\.you'/)
  assert.doesNotMatch(settingsPage, /https:\/\/generativelanguage\.googleapis\.com/)
  assert.doesNotMatch(settingsPage, /https:\/\/api\.openai\.com/)
  assert.doesNotMatch(settingsPage, /https:\/\/ark\.cn-beijing\.volces\.com/)
...
test('settings page offers official default model IDs', () => {
  ...
  assert.match(settingsPage, /gpt-5\.6-sol/)
...
test('settings page fetches and multi-selects upstream models', () => {
  assert.match(settingsPage, /获取模型/)
  assert.match(settingsPage, /modelOptions\.length/)
  assert.match(settingsPage, /type="checkbox"/)
  assert.match(settingsPage, /aiConfigAPI\.models/)
  assert.match(useApi, /models: \(d: any\) => api\.post<\{ models: string\[\] \}>\('\/ai-configs\/models'/)
})
